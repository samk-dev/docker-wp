...
echo '⚙️  INFO: Running init.sh...'

echo '⏳ INFO: checking the .env file to see where the script is being run from ⌛️'
...
if [ -f .env ]; then
  ...
  echo "💥 ERROR: Could not find the .env file? Make sure your running the script from the project root"
  exit 1;  
fi
